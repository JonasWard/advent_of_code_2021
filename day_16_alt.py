...
class Package:
    def __init__(self, hexstream):
        self.bit_string = []
        for c in hexstream:
            self.bit_string.extend(bin(int(c, 16))[2:].zfill(4))
    ...
    def get_bits(self, n):
        self.read += n
        bits = self.bit_string[:n]
        self.bit_string = self.bit_string[n:]
        return bits
...
if __name__ == "__main__":
    with open("/Users/jonasvandenbulcke/Documents/reps/advent_of_code_2021/data/day_16.txt") as fh:
        for line in fh:
            message = line.strip()
